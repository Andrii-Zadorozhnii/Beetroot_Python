...
async def save_to_db(user_id: int):
    """Сохранение данных груза в базу данных и отправка в группу водителей"""
    conn = await asyncpg.connect(**DB_CONFIG)
    ...
    try:
        ...
        driver_message = (
            f"🚛 *Новый груз доступен для перевозки!*\n\n"
            f"📦 *Груз:* {data['name']}\n"
            ...
            f"Для принятия заказа используйте номер перевозки."
        )
            ...
            inline_keyboard=[
                [
                    types.InlineKeyboardButton(
                        text="✅ Принять заказ",
                        callback_data=f"accept_{data['shipment_id']}"
                    )
                ]
            ]
        )
        ...
        # Если есть изображение, отправляем его тоже
        if data.get('image'):
            try:
                await bot.send_photo(
                    chat_id=DRIVERS_GROUP_ID,
                    photo=data['image'],
                    caption=f"Фото груза: {data['name']}"
                )
            except Exception as e:
                logger.error(f"Не удалось отправить изображение: {e}")

    except Exception as e:
        logger.error(f"Ошибка при сохранении груза: {e}")
        raise
    finally:
        await conn.close()
...
@dp.message(lambda message: message.text.startswith("Принять"))
async def accept_cargo(message: types.Message):
    """Обработчик для водителей, принимающих груз"""
    try:
        # Извлекаем номер перевозки из сообщения
        shipment_id = message.text.split()[-1].strip()

        # Получаем информацию о грузе
        conn = await asyncpg.connect(**DB_CONFIG)
        cargo = await conn.fetchrow(
            "SELECT * FROM cargo_cargo WHERE shipment_id = $1",
            shipment_id
        )

        if not cargo:
            await message.reply("Груз не найден!")
            return
        ...
        # Формируем ответ
        response = (
            f"✅ *Груз принят в работу!*\n\n"
            f"👤 *Водитель:* {message.from_user.full_name}\n"
            f"📞 *Контакт:* @{message.from_user.username or 'нет'}\n\n"
            f"📦 *Груз:* {cargo['name']}\n"
            f"📍 *Маршрут:* {cargo['origin']} → {cargo['destination']}\n"
            f"🏢 *Компания:* {cargo['company']}\n"
        ...
        # Можно также отправить уведомление менеджеру
        await bot.send_message(
            chat_id=cargo['user_id'],  # Если user_id хранится как chat_id менеджера
            text=f"Ваш груз {shipment_id} принят водителем {message.from_user.full_name}"
        )

    except Exception as e:
        logger.error(f"Ошибка при принятии груза: {e}")
...
async def process_accept(callback: types.CallbackQuery):
    """Обработчик нажатия кнопки принятия заказа"""
    shipment_id = callback.data.replace('accept_', '')
    ...
    try:
        ...
        response = (
            ...
            f"📞 *Контакт:* @{user.username or 'нет'}\n\n"
            f"📦 *Груз:* {cargo['name']}\n"
            f"📍 *Маршрут:* {cargo['origin']} → {cargo['destination']}\n"
            f"🏢 *Компания:* {cargo['company']}\n"
            f"💰 *Оплата:* {cargo['payment']} USD\n"
        ...
        await bot.send_message(
            chat_id=cargo['user_id'],  # Если user_id хранится как chat_id менеджера
            text=f"Ваш груз {shipment_id} принят водителем {user.full_name}"
        )

    except Exception as e:
        logger.error(f"Ошибка при обработке принятия груза: {e}")
